...
export default function AgendaPage() {
  const [equipamentos, setEqs] = React.useState<Equip[]>([]);
  const [equipamentoId, setEqId] = React.useState<number | ''>('');
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (equipamentoId !== '') p.set('equipamentoId', String(equipamentoId));
      if (start) p.set('start', start);
      if (end) p.set('end', end);
      const r = await fetch('/api/reservas?' + p.toString());
      if (r.ok) setList(await r.json());
    })();
  }, [equipamentoId, start, end]);
    ...
    <main className="max-w-6xl mx-auto p-4 space-y-4">
      ...
      <div className="bg-white text-black rounded-xl p-4 shadow grid md:grid-cols-4 gap-3">
        ...
        <div className="flex items-end">
          <a className="px-4 py-2 rounded border" href="/reservas/nova">
            Novo agendamento
          </a>
        </div>
      </div>
      ...
      <div className="bg-white text-black rounded-xl p-4 shadow">
        ...
        <div className="grid gap-2">
          {list.length === 0 && (
            <div className="text-sm text-gray-600">Sem reservas no período.</div>
          )}
          {list.map((r) => (
            <div key={r.id} className="border rounded-lg p-3 text-sm flex justify-between">
              <div>
                <div>
                  <span className="font-medium">Equipamento:</span> {r.equipamento_nome} (#
                  {r.equipamento_id})
                </div>
                <div>
                  <span className="font-medium">Quando:</span> {fmt(r.horario_inicio)} →{' '}
                  {fmt(r.horario_fim)}
                </div>
                <div>
                  <span className="font-medium">Responsável:</span> {r.responsavel} (#{r.usuario_id}
                  )
                </div>
              </div>
              <span className="text-xs px-2 py-1 rounded bg-green-100 text-green-700 self-start">
                ATIVA
              </span>
            </div>
          ))}
